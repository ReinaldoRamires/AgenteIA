...
def load_data_from_db():
    ...
    try:
        ...
        if not db_url:
            st.error("URL do banco de dados não encontrada."); return [pd.DataFrame()]*3
        ...
        if "cache_market" in pd.read_sql("SELECT name FROM sqlite_master WHERE type='table';", engine)['name'].tolist():
        ...
        st.error(f"Erro ao carregar dados do banco de dados: {e}"); return [pd.DataFrame()]*3
...
if projects_filtered.empty:
    ...
    with tab1:
        ...
        col3.metric("Total de Tarefas", tasks[tasks['project_id'].isin(projects_filtered['id'])].shape[0])
        ...
        with col_chart1:
            if not tasks.empty:
                tasks_per_project = tasks[tasks['project_id'].isin(projects_filtered['id'])].groupby('project_id').size().reset_index(name='task_count')
                tasks_per_project = tasks_per_project.merge(projects_filtered[['id', 'name']], left_on='project_id', right_on='id')
                fig_bar = px.bar(tasks_per_project, x='name', y='task_count', title="Tarefas por Projeto", labels={'name': 'Projeto', 'task_count': 'Qtd. Tarefas'})
                st.plotly_chart(fig_bar, use_container_width=True)

        with col_chart2:
            status_counts_df = projects_filtered['status'].value_counts().reset_index()
            status_counts_df.columns = ['status', 'count']
            fig_pie = px.pie(status_counts_df, names='status', values='count', title="Projetos por Status")
            st.plotly_chart(fig_pie, use_container_width=True)

        with st.expander("Ver Tabela de Projetos Completa"):
            st.dataframe(projects_filtered[['name', 'slug', 'project_type', 'country', 'status']], use_container_width=True)

    with tab2:
        st.subheader("Análise Detalhada por Projeto")
        selected_project_name = st.selectbox("Selecione um projeto para ver os detalhes:", projects_filtered['name'])

        if selected_project_name:
            project_details = projects_filtered[projects_filtered['name'] == selected_project_name].iloc[0]
            ...
            if not project_tasks.empty:
                st.dataframe(project_tasks[['template', 'dor', 'dod', 'estimate']], use_container_width=True)
            else:
            ...
            if not project_market_analysis.empty:
                ...
                latest_analysis_data = json.loads(project_market_analysis.iloc[-1]['data'])
                st.json(latest_analysis_data)
            else:
                st.info(f"Nenhuma análise de mercado encontrada em cache para o tipo '{project_details['project_type']}' e país '{project_details['country']}'.")