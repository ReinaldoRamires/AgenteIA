...
def get_config():
    ...
    try:
        ...
        console.print("[bold red]ERRO FATAL:[/bold red] 'config.yaml' não encontrado. O agendador não pode iniciar.")
        return None
...
def run_status_check(config):
    ...
    try:
        collector = StatusCollector(api_key=config["api_keys"]["notion"], tasks_db_id=config["notion_db"]["tasks_db_id"])
        collector.check_for_updates()
    except Exception as e:
        console.print(f"[bold red]Erro na tarefa de verificação de status:[/bold red] {e}")
