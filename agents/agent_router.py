...
class AgentRouter:
    ...
    def __init__(self, agents: dict, rules: dict, dry_run: bool = False):
        ...
        console.print(
            f"✅ [Agent Router] Maestro dinâmico pronto. Modo dry_run: {self.dry_run}"
        )
    ...
    def _execute_workflow(self, workflow_steps: list, project):
        ...
        console.rule(
            f"[bold magenta]{title_action} Cascata de Análise Dinâmica ({total_steps} etapas)[/bold magenta]"
        )
        ...
        for i, agent_action in enumerate(workflow_steps, 1):
            ...
            if not self.dry_run:
                ...
                if not agent_instance:
                    console.print(
                        f"[bold red]Aviso:[/bold red] Agente para a ação '{agent_action}' não encontrado."
                    )
                    continue

                action_map = {
                    "analyze_market": "analyze_market_potential",
                    "analyze_compliance": "analyze_compliance_risks",
                    "plan_gtm": "generate_strategy",
                    "analyze_risks": "analyze_risks",
                    "design_org": "design_team_structure",
                }
                method_name = action_map.get(agent_action)
                if not method_name:
                    console.print(
                        f"[bold red]Aviso:[/bold red] Método para a ação '{agent_action}' não mapeado."
                    )
                    continue
                ...
                if method_to_call and callable(method_to_call):
                    try:
                        ...
                        console.print(
                            f"[bold red]Falha na execução da ação '{agent_action}': {e}[/bold red]"
                        )
                else:
                    console.print(
                        f"[bold red]Aviso:[/bold red] Método '{method_name}' não encontrado no agente."
                    )
    ...
    def route_event(self, event_type: str, data: dict):
        ...
        """
        console.print(
            f"🧠 [Agent Router] Evento recebido: [bold yellow]{event_type}[/bold yellow]"
        )
        if event_type in self.workflows:
            ...
            if project or self.dry_run:
                ...
                console.print(
                    f"[bold red]Erro:[/bold red] Dados do projeto ausentes para o evento '{event_type}'."
                )
        else:
            console.print(
                f"-> Nenhum fluxo de trabalho definido para o evento '{event_type}'."
            )
